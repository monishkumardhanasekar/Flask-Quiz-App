    ...
    if request.method == "POST":
        ...
        if action == "submit":
            ...
            if is_temp_id:
                ...
                user_id = quiz_data.get('user_id')  # Get user_id from session
                
                # Get user_id from header if not in session (for backward compatibility)
                if user_id is None:
                    user_id = get_user_id()
                
                attempt = QuizAttempt(
                    user_id=user_id,  # Set user_id from session/header
